...
pub const TREASURY_WALLET: Pubkey = pubkey!("TREASURY_WALLET_PUBKEY_HERE"); // TODO: Replace with actual treasury wallet
pub const FEE_BASIS_POINTS: u64 = 300; // 3% = 300 basis points (100 basis points = 1%)
...
pub mod tic_tac_toe {
    ...
    pub fn initialize_game(
        ...
        game.treasury_fee_collected = 0;

        if bet_amount > 0 {
            // Transfer bet from player1 to game account
    ...
    pub fn join_game(ctx: Context<JoinGame>) -> Result<()> {
        ...
        if game.bet_amount > 0 {
            // Transfer bet from player2 to game account
            let cpi_context = CpiContext::new(
                ctx.accounts.system_program.to_account_info(),
                anchor_lang::system_program::Transfer {
    ...
    pub fn make_move(ctx: Context<MakeMove>, position: u8) -> Result<()> {
        ...
        if let Some(winner) = check_winner(&game.board) {
            ...
            if game.bet_amount > 0 {
                ...
                let treasury_fee = total_pot * FEE_BASIS_POINTS / 10000;
                let winner_payout = total_pot - treasury_fee;
                
                game.treasury_fee_collected = treasury_fee;
                ...
                **game.to_account_info().try_borrow_mut_lamports()? -= winner_payout;
                
                if winner == Player::X {
                    **ctx.accounts.player1.try_borrow_mut_lamports()? += winner_payout;
                } else {
                    **ctx.accounts.player2.try_borrow_mut_lamports()? += winner_payout;
                }

                msg!("Game completed: Winner payout: {}, Treasury fee: {}", winner_payout, treasury_fee);
            }
        } else if is_board_full(&game.board) {
            game.status = GameStatus::Draw;
            
            if game.bet_amount > 0 {
                // In case of draw, return bets minus small treasury fee
                let total_pot = game.bet_amount * 2;
                let treasury_fee = total_pot * 100 / 10000; // 1% fee on draws
                let player_refund = (total_pot - treasury_fee) / 2;
                ...
                **game.to_account_info().try_borrow_mut_lamports()? -= player_refund * 2;
                **ctx.accounts.player1.try_borrow_mut_lamports()? += player_refund;
                **ctx.accounts.player2.try_borrow_mut_lamports()? += player_refund;

                msg!("Game draw: Player refunds: {} each, Treasury fee: {}", player_refund, treasury_fee);
            }
        } else {
    ...
    // Admin function to update treasury wallet (only current treasury can call)
    pub fn update_treasury(ctx: Context<UpdateTreasury>, new_treasury: Pubkey) -> Result<()> {
        ctx.accounts.config.treasury_wallet = new_treasury;
        msg!("Treasury wallet updated to: {}", new_treasury);
        Ok(())
    }

    // Admin function to update fee percentage (only treasury can call)
    pub fn update_fee(ctx: Context<UpdateFee>, new_fee_basis_points: u64) -> Result<()> {
        require!(new_fee_basis_points <= 1000, GameError::FeeTooHigh); // Max 10%
        ctx.accounts.config.fee_basis_points = new_fee_basis_points;
        msg!("Fee updated to: {} basis points", new_fee_basis_points);
        Ok(())
    }
}
...
pub struct MakeMove<'info> {
    ...
    /// CHECK: This is the treasury wallet for fee collection
    #[account(mut, address = TREASURY_WALLET)]
...
pub struct UpdateTreasury<'info> {
    ...
    #[account(constraint = authority.key() == config.treasury_wallet)]
